...
function addPosition() {
  ...
  if (state.some(item => item.position === position)) {
    els.formMessage.textContent = 'That position is already on the list.';
    els.input.select();
    return;
  }

  state.push({ position, fiber: 0, orange: 0, green: 0 });
...
function pulseMessage(message, isError = false) {
  els.formMessage.style.color = isError ? 'var(--danger)' : 'var(--green)';
...
function setCount(position, type, value) {
  const item = state.find(entry => entry.position === position);
  if (!item) return;
  const parsed = Number.parseInt(value, 10);
  item[type] = Number.isFinite(parsed) ? Math.max(0, parsed) : 0;
  saveState();
}
...
function deletePosition(position) {
  const index = state.findIndex(entry => entry.position === position);
  if (index === -1) return;
...
function render() {
  ...
  state.forEach(item => {
    ...
    node.querySelector('.position-code').textContent = item.position;
    node.querySelector('.delete-position').addEventListener('click', () => deletePosition(item.position));
    ...
    card.querySelectorAll('.compact-counter').forEach(counter => {
      ...
      input.value = item[type];
      input.addEventListener('input', () => setCount(item.position, type, input.value));
      input.addEventListener('blur', () => {
        const current = state.find(entry => entry.position === item.position);
        input.value = current ? current[type] : 0;
      });
      counter.querySelector('.minus').addEventListener('click', () => updateCount(item.position, type, -1, input));
      counter.querySelector('.plus').addEventListener('click', () => updateCount(item.position, type, 1, input));
    });

    els.list.appendChild(node);
  });

  els.positionCount.textContent = `${state.length} position${state.length === 1 ? '' : 's'}`;
}

function buildExportText() {
  if (!state.length) return 'No positions added.';

  return [...state].sort(comparePositions).map(item =>
...
function showPreview() {
  ...
  else alert(els.exportText.value);
}
...
function downloadText() {
  ...
  a.href = url;
  a.download = `toa-receiving-${date}.txt`;
  a.click();
...
els.copy.addEventListener('click', () => copyExportText(els.copy));
els.dialogCopy.addEventListener('click', () => copyExportText(els.dialogCopy));
els.download.addEventListener('click', downloadText);
els.theme.addEventListener('click', () => document.body.classList.toggle('high-contrast'));
